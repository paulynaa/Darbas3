...
void failuskaick(string wp, Pazymiai hi,  list<Pazymiai>& P, list<Pazymiai>& Z){
...
void failuskaickstrategija1(string wp, Pazymiai hi,std::list<Pazymiai>& S, std::list<Pazymiai>& P, std::list<Pazymiai>& Z){
...
/*
void failuskaickstrategija2(string wp, Pazymiai hi, std::list<Pazymiai>& P, std::list<Pazymiai>& Z){
...
*/
void failuskaickstrategija2(string wp, Pazymiai hi, list<Pazymiai>& P, list<Pazymiai>& Z) {
    ifstream file(wp);
int z;
    if (!file.is_open()) {
        cout << "Failas neegzistuoja!" << endl;
    ...
    for (int l = 0; l < lineCount - 1; l++) {
        ...
        while (it != P.end()) {
            if (it->galutinis < 5) {
                Z.push_back(*it);
                it = P.erase(it); // Erase the current student from P and update the iterator
            } else {
...
void failuskaickstrategija3(string wp, Pazymiai hi, std::list<Pazymiai>& S, std::list<Pazymiai>& P, std::list<Pazymiai>& Z){
