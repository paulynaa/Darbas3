...
protected:
    ...
    std::string pav_;
public:
    virtual ~Zmogus() {} // Padarome bazinæ klasæ abstrakèià su virtualiu destruktoriumi

    // Metodai skirti gauti ir nustatyti vardà ir pavardæ
...
public:
    ...
    // Metodai skirti nustatyti paþymiø vidurká, egzamino rezultatà, gauti paþymius ir t.t.
    void setVid(double newVid) { vid_ = newVid; }
    void setEgz(int newEgz) { egz_ = newEgz; }
    void setOnePaz(int newPaz) { paz_.push_back(newPaz); }
    void setPazymiai(const std::vector<int>& pazymiai) { paz_ = pazymiai; }
    ...
    // Implementuojame abstrakèiø metodø nustatymà ir gavimà
