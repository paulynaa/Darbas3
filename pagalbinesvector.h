...
private:
    ...
    Vector<int> paz_; /**< Pazymiu sarasas */
...
public:
     ...
     * @param med Mediana
     */
    Pazymiai(const std::string& var, const std::string& pav, double vid, int egz, const Vector<int>& paz,
    ...
    * @brief Grazina nurodyta pazymi is saraso pagal pozicija.*/
    int getPazN(const Vector<int>& newPaz, int pos) const { return newPaz[pos]; }
    ...
    * @brief Nustato pazymiu sarasa.*/
    void setPazymiai(const Vector<int>& pazymiai) { paz_ = pazymiai; }
    ...
    * @brief Grazina pazymiu sarasa.*/
    Vector<int> getPaz() const { return paz_; }
